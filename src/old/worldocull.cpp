...
void computeraytable(float vx, float vy)
{
    ...
    // must rewrite this function. Instead of really doing occlusion culling, I can 
    // just calculate how long each ray should be depending on the camera pitch/yaw
    // and where the ray would hit the frustum.
    //
    // I don't even need this function any more, just change how isoccluded(...) works,
    // or turn off culling....that is default now!

    odist = getvar("fog")*1.5f;

    float apitch = (float)fabs(camerapitch);
    float af = getvar("fov")/2+apitch/1.5f+3;
    float byaw = (camerayaw-90+af)/360*PI2;
    float syaw = (camerayaw-90-af)/360*PI2;
