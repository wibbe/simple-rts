...
void gl_drawhud(int w, int h, int mouse_x, int mouse_y, int curfps, int nquads, int curvert, bool underwater)
{
    readmatrices();
    if(editmode)
    {
    ...
    if(!rendermenu())
    {
        ...
        float mouse_virt_x = ((float)mouse_x / w) * VIRTW;
        float mouse_virt_y = ((float)mouse_y / h) * VIRTH;

        conoutf("mouse: %f, %f", mouse_virt_x, mouse_virt_y);

        glTexCoord2d(0.0, 0.0); glVertex2f(mouse_virt_x - chsize, mouse_virt_y - chsize);
        glTexCoord2d(1.0, 0.0); glVertex2f(mouse_virt_x + chsize, mouse_virt_y - chsize);
        glTexCoord2d(1.0, 1.0); glVertex2f(mouse_virt_x + chsize, mouse_virt_y + chsize);
        glTexCoord2d(0.0, 1.0); glVertex2f(mouse_virt_x - chsize, mouse_virt_y + chsize);
