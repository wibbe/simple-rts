...
namespace tcl {
  ...
  static bool applyOperand(Operand * op, std::stack<double> & values, std::stack<Operand *> & operands)
  {
    ...
    else if (op->op[0] == ')')
    {
      ...
      if (!top || top->op[0] != '(')
      {
        _reportError("Stack error, no matching \'(\'");
        return false;
      }
  ...
  double calculateExpr(std::string const& str)
  {
      ...
      if (!digit)
      {
        ...
        else if (Operand * op = getOperand(it))
        {
          if (lastOp && (lastOp == &startOp || lastOp->op[0] == ')'))
          {
            ...
            {
              _reportError("Illegal use of operator (" + std::string(op->op) + ")");
              return 0.0;
            }
          }
        ...
        {
          _reportError("Syntax error in expr '" + str + "'");
          return 0.0;
        }
      }
      else
    ...
    if (values.size() != 1)
    {
      _reportError("Error while executing expr '" + str + "'");
      return 0.0;
    }
